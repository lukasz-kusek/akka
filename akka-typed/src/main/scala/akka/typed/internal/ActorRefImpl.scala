 ...
 */
private[typed] final class FunctionRef[-T](_path: a.ActorPath,
 ...
 *
 * FIXME does this need a maximum buffer length?
 */
private[typed] class FutureRef[-T](_p: a.ActorPath, f: Future[ActorRef[T]]) extends WatchableRef[T](_p) {
  import FutureRef._
        ...
        case l @ Left(list) =>
          list.synchronized {
            ...
            if (unsafe.compareAndSwapObject(this, targetOffset, l, Right(ref)))
              ref.toImpl.sendSystem(Watch(ref, this))
  ...
  override def terminate(): Unit = {
    ...
    old match {
      case Right(target: ActorRef[_]) => target.toImpl.sendSystem(Unwatch(target, this))
