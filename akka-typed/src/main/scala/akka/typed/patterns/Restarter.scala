 ...
 */
case class Restarter[T, Thr <: Throwable: ClassTag](behavior: () ⇒ Behavior[T], resume: Boolean) extends Behavior[T] {
...
object Restarter {
  class Apply[Thr <: Throwable](c: ClassTag[Thr], resume: Boolean) {
    def wrap[T](p: Props[T]) = Props(() ⇒ Restarter(p.creator, resume)(c), p.dispatcher, p.queueSize)
  }
